...
import { getAllSongsByPlaylistId } from "@/lib/actions/playlist.actions";
...
const MusicPage = ({ params }: Props) => {
  ...
  const currentPlaylistId = getIntOrUndefined(playlistId);
  ...
  const fetchAllSongs = async () => {
    ...
    try {
      const res = await getAllSongsByPlaylistId(currentPlaylistId);
      if (!res.status) {
